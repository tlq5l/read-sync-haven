...
export function AnimationProvider({ children }: { children: React.ReactNode }) {
  ...
  const createTransitionGroup = useCallback((id: string, timeout = ANIMATION_TIMINGS.normal) => {
  ...
  const registerTransitionElement = useCallback((groupId: string, elementId: string) => {
    setTransitionGroups((prev) => {
  ...
  const unregisterTransitionElement = useCallback((groupId: string, elementId: string) => {
    setTransitionGroups((prev) => {
      const groupIndex = prev.findIndex((group) => group.id === groupId);
  ...
  const triggerTransition = useCallback((groupId: string) => {
    setTransitionGroups((prev) => {
  ...
  const synchronizeAnimations = useCallback((callback: () => void) => {
    if (animationFrame !== null) {
      cancelAnimationFrame(animationFrame);
    }
