...
import {
	MockArticleProvider,
	mockRawArticles, // Import mock data if needed for assertions
	testUpdateFilters,
	testSetSort,
	testToggleSortDirection,
    MockArticleContext, // Import mock context for useArticles mock
} from "../test-utils/MockArticleProvider"; // Adjusted path
import type { Article, Tag } from "@/services/db"; // Keep necessary type imports
import type { ArticleFilters, ArticleSortField, SortCriteria } from "@/types/articles"; // Keep necessary type imports
...
vi.mock("@/components/ui/select", async () => ({
	Select: ({
		...
		<div className="mock-select" data-testid="mock-select" data-value={value}>
			<button type="button" onClick={() => onValueChange("title")} data-testid="select-option-title">Sort by Title</button>
			<button type="button" onClick={() => onValueChange("savedAt")} data-testid="select-option-savedAt">Sort by Date</button>
			<button type="button" onClick={() => onValueChange("siteName")} data-testid="select-option-siteName">Sort by Source</button>
			<button type="button" onClick={() => onValueChange("estimatedReadTime")} data-testid="select-option-readTime">Sort by Read Time</button>
		</div>
	),
	SelectTrigger: ({ children }: { children: React.ReactNode }) => <div data-testid="select-trigger">{children}</div>,
	SelectValue: ({ placeholder }: { placeholder: string }) => <span>{placeholder}</span>,
	SelectContent: ({ children }: { children: React.ReactNode }) => <div>{children}</div>,
	SelectItem: ({ value, children }: { value: string; children: React.ReactNode }) => <div data-value={value} data-testid={`select-item-${value}`}>{children}</div>,
}));
vi.mock("@/components/ui/dropdown-menu", async () => {
    ...
    return {
        ...
        DropdownMenuTrigger: ({ children, asChild, ...props }: { children: React.ReactNode; asChild?: boolean; [key: string]: any }) => {
            return asChild && React.isValidElement(children) ? React.cloneElement(children, props) : <div className="mock-dropdown-trigger" {...props}>{children}</div>;
        },
        DropdownMenuContent: ({ children }: { children: React.ReactNode }) => <div className="mock-dropdown-content">{children}</div>,
        DropdownMenuLabel: ({ children }: { children: React.ReactNode }) => <div>{children}</div>,
        DropdownMenuSeparator: () => <div className="mock-separator" />,
        DropdownMenuItem: ({ children, ...props }: { children: React.ReactNode; [key: string]: any }) => <div className="mock-dropdown-item" {...props}>{children}</div>,
        DropdownMenuCheckboxItem: ({ children, checked, onCheckedChange, ...props }: { children: React.ReactNode; checked: boolean; onCheckedChange: (checked: boolean) => void; [key: string]: any }) => (
            <div className="mock-checkbox-item" {...props} onClick={() => onCheckedChange(!checked)}>
                <input type="checkbox" checked={checked} readOnly data-testid={`checkbox-${String(children).toLowerCase()}`} />
                <span>{children}</span>
            </div>
        ),
    };
...
vi.mock("@/context/ArticleContext", async (importOriginal) => {
	const original = await importOriginal<typeof import("@/context/ArticleContext")>();
    const { MockArticleContext } = await import("../test-utils/MockArticleProvider"); // Import mock context
...
const renderInboxPage = () => {
	return render(
		<MemoryRouter>
			<MockArticleProvider> {/* Use the imported provider */}
				<InboxPage />
			</MockArticleProvider>
		</MemoryRouter>,
	);
...
describe("InboxPage Integration Tests", () => {
	...
	it("should show filtered empty state and allow clearing filters", async () => {
        ...
        if (clearButton) {
            ...
            console.warn("Clear Filters button not found in rendered output for this test.");
        }
	});
});
