...
describe("lib/articleUtils", () => {
  ...
  describe("runOneTimeFileSync", () => {
    ...
    const mockUser = { primaryEmailAddress: { emailAddress: "test@example.com" } } as unknown as any;
    ...
    it("should migrate EPUBs with content to fileData field and update them", async () => {
      ...
      expect(db.updateArticle).toHaveBeenCalledWith(expect.objectContaining({
        _id: "epub1",
        _rev: "rev1",
        fileData: expect.stringContaining("data:application/epub+zip;base64,"), // Base64 moved from content to fileData
