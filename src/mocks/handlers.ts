...
export const handlers: RequestHandler[] = [
  ...
  http.post(`${WORKER_BASE_URL}/api/summarize`, async ({ request }) => {
    ...
    try {
      ...
      if (!body || typeof body !== 'object' || !('content' in body)) {
        ...
        return new HttpResponse(JSON.stringify({ error: 'Bad Request: Missing content' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      ...
      return new HttpResponse(JSON.stringify({ error: 'Bad Request: Invalid JSON' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
  ...
  http.post(`${WORKER_BASE_URL}/api/chat`, async ({ request }) => {
    ...
    try {
      const body = await request.json();
      if (!body || typeof body !== 'object' || !('content' in body) || !('message' in body)) {
        console.log('[MSW] Bad request body for /api/chat');
        return new HttpResponse(JSON.stringify({ error: 'Bad Request: Missing content or message' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
        });
      }
  ...
  http.post('http://fake-gcf/summarize', async ({ request }) => {
  ...
  http.post('http://fake-gcf/chat', async ({ request }) => {
  ...
  http.post('http://fake-gcf.test/summarize', async ({ request }) => {
  ...
  http.post('http://fake-gcf.test/chat', async ({ request }) => {
