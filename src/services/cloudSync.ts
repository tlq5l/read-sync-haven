...
export async function saveItemToCloud(article: Article): Promise<boolean> {
  ...
  try {
    const response = await fetch('https://bondwise-sync-api.vikione.workers.dev/items', {
      method: 'POST',
      headers: {
 ...
 */
export async function importCloudItems(email: string, clerkUserId: string): Promise<number> {
  try {
 ...
 */
function mapArticleType(type: Article['type']): "article" | "youtube" | "other" {
  switch (type) {
    case 'article':
      return 'article';
    case 'pdf':
