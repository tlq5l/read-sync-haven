...
import { Shortcut, ShortcutGroup, formatShortcut } from '@/lib/keyboard-shortcuts';
...
export function ShortcutCategory({ title, description, shortcuts }: ShortcutCategoryProps) {
  return (
...
export function ShortcutDisplay({ shortcuts, className }: ShortcutDisplayProps) {
  ...
  const categories = Object.keys(groupedShortcuts).map((category) => {
    ...
    const groupInfo = shortcutGroups.find((group) => group.category === category);
    
    return {
      category,
      title: groupInfo?.title || category.charAt(0).toUpperCase() + category.slice(1),
