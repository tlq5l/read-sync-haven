...
>(({ className, ...props }, ref) => (
	<ToastPrimitives.Viewport
		ref={ref}
		className={cn(
			"fixed top-0 z-[100] flex max-h-screen w-full flex-col-reverse p-4 sm:bottom-0 sm:right-0 sm:top-auto sm:flex-col md:max-w-[420px] gpu-accelerated",
			className,
		)}
		{...props}
	/>
...
const toastVariants = cva(
	"group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all gpu-accelerated toast " +
	...
	<ToastPrimitives.Action
		...
		className={cn(
			"inline-flex h-8 shrink-0 items-center justify-center rounded-md border bg-transparent px-3 text-sm font-medium " +
				"ring-offset-background transition-colors duration-200 ease-smooth-in-out " +
				...
				"group-[.destructive]:focus:ring-destructive gpu-accelerated",
			className,
		)}
		{...props}
	/>
	...
	<ToastPrimitives.Close
		...
		className={cn(
			"absolute right-2 top-2 rounded-md p-1 text-foreground/50 opacity-0 " +
				"transition-opacity duration-200 ease-smooth-in-out " +
				...
				"group-[.destructive]:focus:ring-red-400 group-[.destructive]:focus:ring-offset-red-600 gpu-accelerated",
			className,
		)}
		toast-close=""
		{...props}
