...
import { renderHook, act } from '@testing-library/react'; // Removed waitFor
import { useChat, type ChatMessage } from './useChat';
import { useAuth } from '@clerk/clerk-react';
import { server } from '../mocks/server'; // MSW server
import { http, HttpResponse } from 'msw';
import { QueryClientWrapper, getTestQueryClient } from '../test-utils/QueryClientWrapper'; // Import the wrapper
...
const WORKER_CHAT_URL = 'https://bondwise-sync-api.vikione.workers.dev/api/chat';
...
describe('useChat Hook', () => {
	...
	it('should initialize with correct default states', () => {
		const { result } = renderHook(() => useChat(MOCK_ARTICLE_CONTENT), { wrapper });
	...
	it('should set error if content is null when handleChatSubmit is called', () => {
		...
		expect(result.current.chatError?.message).toBe('Article content not yet extracted or available.');
	...
	it('should return error if getToken fails', async () => {
		...
		await act(async () => {
			...
			await expect(result.current.chatMutation.mutateAsync(userMessage)).rejects.toThrow(
		...
		expect(result.current.isChatting).toBe(false);
		expect(result.current.chatError).toBeInstanceOf(Error);
		expect(result.current.chatError?.message).toBe('User not authenticated (Clerk token missing).');
		// Check history includes the AI error message added by onError
		expect(result.current.chatHistory.some(m => m.sender === 'ai' && m.text.includes('User not authenticated'))).toBe(true);
	...
	it('should handle unauthorized (401) error from worker', async () => {
		server.use(
			http.post(WORKER_CHAT_URL, () => {
				return new HttpResponse(JSON.stringify({ error: 'Mock Chat Unauthorized' }), { status: 401 });
			}),
		);
		const { result } = renderHook(() => useChat(MOCK_ARTICLE_CONTENT), { wrapper });
		const userMessage = 'Test 401 message';
		const expectedError = /Mock Chat Unauthorized|Chat request failed with status 401/;

		await act(async () => {
			result.current.setChatInput(userMessage);
			await expect(result.current.chatMutation.mutateAsync(userMessage)).rejects.toThrow(expectedError);
		});

		expect(result.current.isChatting).toBe(false);
		expect(result.current.chatError).toBeInstanceOf(Error);
		expect(result.current.chatError?.message).toMatch(expectedError);
		expect(result.current.chatHistory.some(m => m.sender === 'ai' && m.text?.match(expectedError))).toBe(true);
		expect(mockGetToken).toHaveBeenCalledTimes(1);
	});
	...
	it('should handle server error (500) from worker', async () => {
		server.use(
			http.post(WORKER_CHAT_URL, () => {
				return new HttpResponse(JSON.stringify({ error: 'Mock Server Error' }), { status: 500 });
			}),
		);
		const { result } = renderHook(() => useChat(MOCK_ARTICLE_CONTENT), { wrapper });
		const userMessage = 'Test 500 message';
		const expectedError = /Mock Server Error|Chat request failed with status 500/;
	...
	it('should handle response missing response field', async () => {
		...
		const expectedError = 'Invalid response from chat service (missing response).';
		...
		expect(result.current.chatHistory.some(msg => msg.sender === 'ai' && msg.text === `Error: ${expectedError}`)).toBe(true);
		expect(mockGetToken).toHaveBeenCalledTimes(1);
	});
});