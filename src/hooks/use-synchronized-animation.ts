...
export function useSynchronizedAnimation({
  ...
  const uniqueElementId = useRef(elementId || `element-${Math.random().toString(36).substring(2, 9)}`);
  ...
  }, [groupId, registerTransitionElement, unregisterTransitionElement, disabled]);
  ...
  useEffect(() => {
    ...
    const checkAnimation = () => {
      const newIsAnimating = isElementAnimating(groupId, uniqueElementId.current);
...
export function useTransitionGroup(groupId: string, timeout?: number) {
  const { createTransitionGroup, triggerTransition, synchronizeAnimations } = useAnimation();
